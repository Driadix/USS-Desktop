...
Mutable Arduino CLI state is stored under `%LOCALAPPDATA%\USS Desktop\arduino-cli` by default so network releases do not install cores, libraries, downloads, and build cache over SMB. Set `USS_DESKTOP_LOCAL_DATA_ROOT` to override the local data root.
